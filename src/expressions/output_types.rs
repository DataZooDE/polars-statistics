...
/// Extended output for tests with degrees of freedom
pub fn stats_with_df_output_dtype(_input_fields: &[Field]) -> PolarsResult<Field> {
    let fields = vec![
        Field::new("statistic".into(), DataType::Float64),
...
pub fn generic_stats_output(statistic: f64, p_value: f64, name: &str) -> PolarsResult<Series> {
    ...
    StructChunked::from_series(name.into(), stat_series.len(), [&stat_series, &pval_series].into_iter())
        .map(|ca| ca.into_series())
}

/// Create output with df
pub fn generic_stats_with_df_output(
    statistic: f64,
    df: f64,
    p_value: f64,
    name: &str,
    ...
    let stat_series = Series::new("statistic".into(), vec![statistic]);
    let df_series = Series::new("df".into(), vec![df]);
    let pval_series = Series::new("p_value".into(), vec![p_value]);

    StructChunked::from_series(
        name.into(),
        stat_series.len(),
        [&stat_series, &df_series, &pval_series].into_iter(),
    )
    .map(|ca| ca.into_series())
}
